...
EXPECTED_CHAINS = {
    "development": ["mk-plan", "cook", "code-review", "ship"],
    "bugfix": ["scout", "mk-debug", "fix", "code-review"],
    "investigation": ["scout", "mk-debug", "brainstorm"],
}
...
def _extract_body_refs(content: str, own_name: str) -> set[str]:
    ...
    # Normalize: strip mk: prefix if present in the captured group
    # The regex captures what's after /mk: so "plan" from "/mk:plan"
    refs.discard(own_name)
    # Also discard dir-name form of own name (e.g. "mk-plan" vs "plan")
    refs.discard(own_name.replace("mk:", ""))
    return refs
...
def build_reference_graph(skills: dict[str, dict]) -> dict:
    ...
    for dir_name, data in skills.items():
        for ref in data["body_refs"]:
            # Resolve ref to dir name (refs are like "cook", "mk-plan", etc.)
