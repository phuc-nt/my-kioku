 ...
 * Rendering is config-driven via statuslineLayout in .mk.json.
...
// Read stdin with optional inactivity timeout (MK_STATUSLINE_STDIN_TIMEOUT_MS)
async function readStdin() {
  return new Promise((resolve, reject) => {
    const chunks = [];
    stdin.setEncoding('utf8');
    const parsedTimeout = Number.parseInt(env.MK_STATUSLINE_STDIN_TIMEOUT_MS || '', 10);
