#!/usr/bin/env python3
"""
MyKit Help - All-in-one guide with dynamic skill discovery.
...
def emit_output_type(output_type: str) -> None:
    ...
    print(f"@MK_OUTPUT_TYPE:{output_type}")
    print()
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "ck.json", ".mk.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "coding-level": {
        ...
        "tip": "Set in .mk.json. Guidelines auto-inject on session start",
    },
    "config": {
        "title": "MyKit Configuration (.mk.json)",
        "workflow": [
            ("Global", "Set user prefs in `~/.claude/.mk.json`"),
            ("Local", "Override per-project in `./.claude/.mk.json`"),
...
def show_config_guide() -> None:
    """Display comprehensive .mk.json configuration guide."""
    emit_output_type("comprehensive-docs")

    print("# MyKit Configuration (.mk.json)")
    print()
    print("**Locations (cascading resolution):**")
    print("- Global: `~/.claude/.mk.json` (user preferences)")
    print("- Local: `./.claude/.mk.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.claude/.mk.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.claude/.mk.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.claude/.mk.json - applies everywhere")
    ...
    print("# Local (.claude/.mk.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.claude/.mk.json` - personal preference")
    print("- Local: `./.claude/.mk.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.mk.json`")
...
def main():
    ...
    if input_str.lower() in ["config", "configuration", ".mk.json", "ck.json"]:
        show_config_guide()
        return
