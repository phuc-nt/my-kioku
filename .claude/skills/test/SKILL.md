...
Browser-based visual testing via `mk:agent-browser`, `mk:web-testing`, `mk:chrome-profile`, or project-native Playwright/Vitest/k6 commands. Covers screenshots, responsive checks, accessibility audits, form automation, and console error collection.
...
6. For frontend: run UI tests via `mk:agent-browser`, `mk:web-testing`, `mk:chrome-profile`, or project-native browser tests
...
- **Browser**: `mk:agent-browser` for live browser testing without real user cookies; `mk:chrome-profile` for the user's actual Chrome login state; `mk:web-testing` or project-native Playwright/Vitest/k6 for repeatable UI tests
- **Analysis**: `mk:ai-multimodal` skill for screenshot analysis
- **Debugging**: `mk:debug` skill when tests reveal bugs requiring investigation
- **Thinking**: `mk:sequential-thinking` skill for complex test failure analysis
...
**Typically follows:** `/mk:cook` (test after implementation), `/mk:fix` (test after bug fix)
**Typically precedes:** `/mk:code-review` (review after tests pass)
**Related:** `/mk:cook` (implement then test), `/mk:fix` (fix then test)
