...
**Templates:** `mk:research`, `mk:cook`, `mk:code-review`, `mk:debug`
...
```
CK Context:
- Work dir: {MK_PROJECT_ROOT or CWD}
- Reports: {MK_REPORTS_PATH or "plans/reports/"}
- Plans: {MK_PLANS_PATH or "plans/"}
- Branch: {MK_GIT_BRANCH or current branch}
- Naming: {MK_NAME_PATTERN or "YYMMDD-HHMM"}
- Active plan: {MK_ACTIVE_PLAN or "none"}
...
3. **CALL** `TaskCreate` x N -- one per angle:
   ...
   - Description: `Investigate <angle> for topic: <topic>. Save report to: {MK_REPORTS_PATH}/researcher-{N}-{MK_NAME_PATTERN}-{topic-slug}.md. Format: Executive summary, key findings, evidence, recommendations. Mark task completed when done. Send findings summary to lead.`
...
6. **READ** all researcher reports from `{MK_REPORTS_PATH}/`

7. **SYNTHESIZE** into: `{MK_REPORTS_PATH}/research-summary-{MK_NAME_PATTERN}-{topic-slug}.md`
...
3. **CALL** `TaskCreate` x N -- one per focus:
   ...
   - Description: `Review <scope> for <focus>. Output severity-rated findings only. Format: [CRITICAL|IMPORTANT|MODERATE] <finding> -- <evidence> -- <recommendation>. No "seems" or "probably" -- concrete evidence only. Save to: {MK_REPORTS_PATH}/reviewer-{N}-{MK_NAME_PATTERN}-{scope-slug}.md. Mark task completed when done.`
...
6. **SYNTHESIZE** into: `{MK_REPORTS_PATH}/review-{scope-slug}.md`
...
7. **WRITE** root cause report: `{MK_REPORTS_PATH}/debug-{issue-slug}.md`
