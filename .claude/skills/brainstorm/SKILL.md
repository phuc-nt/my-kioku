...
- Use `mk:docs-seeker` skill to read latest documentation of external plugins/packages
- Leverage `mk:ai-multimodal` skill to analyze visual materials and mockups
- Query `psql` command to understand current database structure and existing data
- Employ `mk:sequential-thinking` skill for complex problem-solving that requires structured analysis
...
flowchart TD
    ...
    H --> I{Create Plan?}
    I -->|Yes| J[Invoke /mk:plan]
    I -->|No| K[End Session]
    J --> L[Journal]
    K --> L
```

**This diagram is the authoritative workflow.** If prose conflicts with this flow, follow the diagram. The terminal state is either `/mk:plan` or end.

## Your Process
1. **Scout Phase**: Use `mk:scout` skill to discover relevant files and code patterns, read relevant docs in `<project-dir>/docs` directory, to understand the current state of the project
...
9. **Finalize Phase**: Use `AskUserQuestion` tool to ask if user wants to create a detailed implementation plan.
   - If `Yes`: Run `/mk:plan` command with the brainstorm summary context as the argument to ensure plan continuity.
     **CRITICAL:** The invoked plan command will create `plan.md` with YAML frontmatter including `status: pending`.
   - If `No`: End the session.
10. **Journal Phase**: Run `/mk:journal` to write a concise technical journal entry upon completion.
...
## Output Requirements
**IMPORTANT:** Invoke "/mk:project-organization" skill to organize the reports.
...
**Typically follows:** `/mk:debug` (brainstorm solutions for diagnosed issues), `/mk:scout` (brainstorm after discovery)
**Typically precedes:** `/mk:plan` (plan the agreed solution)
**Related:** `/mk:plan` (plan after brainstorming), `/mk:debug` (debug before brainstorming)
