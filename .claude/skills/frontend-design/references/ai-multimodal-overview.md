...
Entry point for using the `mk:ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `mk:ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `mk:frontend-design` with `mk:ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `mk:chrome-devtools` Skill
Use `mk:chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `mk:media-processing` Skill
