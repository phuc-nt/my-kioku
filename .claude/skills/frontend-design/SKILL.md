...
**All workflows**: Activate `mk:ui-ux-pro-max` skill FIRST for design intelligence.

**Precedence:** When anti-slop rules (below) conflict with `mk:ui-ux-pro-max` recommendations (e.g., Inter font, AI Purple palette, Lucide-only icons), substitute with alternatives from `./references/anti-slop-rules.md` unless the user explicitly requested the conflicting choice.

## Screenshot/Video Replication (Quick Reference)

1. **Analyze** with `mk:ai-multimodal` skill - extract colors, fonts, spacing, effects
...
**Assets**: Generate images with `mk:ai-multimodal`, process with `mk:media-processing`
