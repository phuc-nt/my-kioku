...
Orchestration workflows chaining Stitch design generation with MyKit implementation skills.
...
# 4. Activate mk:frontend-design to implement React components from the spec
```
...
| Full page with style guide | `mk:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `mk:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `mk:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use mk:ui-ux-pro-max for text-based design instead
