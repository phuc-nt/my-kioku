 ...
 *   5. "my-kit-default"     last resort fallback
 */
...
const QUOTA_DIR = path.join(os.homedir(), ".my-kit");
...
if (directProjectId) {
  ...
  projectId = detected || "my-kit-default";
  resolvedProjectName = projectId;
  isNameBasedProject = true;
  if (detected) {
...
async function main() {
  try {
    ...
    if (remaining < creditsNeeded) {
      ...
      console.error("[i] Use mk:ui-ux-pro-max as fallback, or wait until midnight UTC.");
      process.exit(2);
    }
    ...
    if (err.code === "RATE_LIMITED") {
      ...
      q.count = q.limit;
      saveQuota(q);
      console.error("[X] Daily quota exceeded (local tracker synced). Try tomorrow or use mk:ui-ux-pro-max.");
