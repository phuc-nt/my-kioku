...
Profile-aware browser automation path for my-kit Engineer when the agent needs the user's real Chrome state: the right Google account, cookies, workspace, tenant, or logged-in product session.

This is not the default for ordinary browser testing. Use `mk:agent-browser` when a fresh or tool-managed browser is enough. Use project-native Playwright or `mk:web-testing` for repeatable CI tests. Use this skill when the user's actual Chrome profile state matters.
...
That per-machine config survives my-kit updates. Use `chrome-profile setup --yes` for non-interactive bootstrap.
