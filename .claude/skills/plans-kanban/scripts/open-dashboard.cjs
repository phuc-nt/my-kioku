...
function printHelp() {
  ...
  console.log('');
  console.log('Opens the MyKit CLI dashboard at:');
...
function printPlansNotAvailable(port) {
  ...
  process.stderr.write('  The integrated plans dashboard requires my-kit-cli with the plans-dashboard feature.\n');
...
async function main() {
  ...
  if (args.stop) {
    if (stopDashboard()) {
      console.log('Stopped the launcher-managed MyKit dashboard process.');
    } else {
  ...
  if (!dashboardPort) {
    if (!ensureCliExists()) {
      console.error('MyKit CLI not found: `ck` is required to start the dashboard.');
      console.error(`Open ${DASHBOARD_URL} manually after starting \`ck ${START_COMMAND.join(' ')}\`.`);
      process.exitCode = 1;
      return;
    }

    console.log(`Starting MyKit dashboard on http://localhost:${DEFAULT_PORT} ...`);
    startDashboard();
