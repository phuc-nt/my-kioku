...
`mk:plans-kanban` no longer runs a standalone server.

## What changed

- Old behavior: `plans-kanban` started its own HTTP server, renderer, assets bundle, and PID-managed background process.
- New behavior: `plans-kanban` is a thin launcher for the MyKit CLI dashboard at `http://localhost:3456/plans`.
