---
name: mk:plans-kanban
description: Open the MyKit plans dashboard in the CLI config UI. Use for plan kanban views, progress tracking, timeline checks, and quick navigation into plan files.
category: dev-tools
...
Thin launcher for the MyKit CLI dashboard plans view.
...
The dashboard at `/plans` is only opened when the running my-kit-cli instance supports it — detected by either:
...
**`ck` not found**
Install the MyKit CLI and confirm `ck --version` works in your shell.
