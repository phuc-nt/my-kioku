...
Not for: full project cloning (`mk:bootstrap`), simple file copy, or package installation.

## Usage

```text
/mk:xia <github-url|owner/repo|local-path> [feature-description] [--compare|--copy|--improve|--port] [--auto|--fast]
```
...
1. Pack the source with `/mk:repomix`.
...
4. Use `/mk:scout` on the local project to map architecture, similar features, and integration points.
...
- activate `/mk:sequential-thinking` to trace multi-step flows
...
Do not invoke `/mk:brainstorm` from inside `xia`; that skill can create its own planning handoff and break `xia`'s phase ownership.
...
- other modes: present the plan path and hand implementation to `/mk:cook`

Implementation handoff text:

```text
Plan ready at ./plans/<plan-dir>/plan.md. To implement, run /mk:cook <plan-path>.
```
