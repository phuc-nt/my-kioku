...
```
/mk:bootstrap <user-requirements>
```
...
**Example:**
```
/mk:bootstrap "Build a SaaS dashboard with auth" --fast
/mk:bootstrap "E-commerce platform with Stripe" --parallel
```
...
### Planning Phase
Activate **mk:plan** skill with mode-appropriate flag:
- `--full` → `/mk:plan --hard <requirements>` (thorough research + validation)
- `--auto` → `/mk:plan --auto <requirements>` (auto-detect complexity)
- `--fast` → `/mk:plan --fast <requirements>` (skip research)
- `--parallel` → `/mk:plan --parallel <requirements>` (file ownership + dependency graph)

Planning skill outputs a plan path. Pass this to cook.

### Implementation Phase
Activate **mk:cook** skill with the plan path and mode-appropriate flag:
- `--full` → `/mk:cook <plan-path>` (interactive review gates)
- `--auto` → `/mk:cook --auto <plan-path>` (skip review gates)
- `--fast` → `/mk:cook --auto <plan-path>` (skip review gates)
- `--parallel` → `/mk:cook --parallel <plan-path>` (multi-agent execution)
...
- Run `/mk:journal` to write a concise technical journal entry upon completion
