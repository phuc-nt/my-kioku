...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `mk:ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `mk:ai-multimodal` skill
5. Screenshot wireframes with `mk:chrome-devtools` → save to `./docs/wireframes/`
...
Activate **mk:plan** skill: `/mk:plan --hard <requirements>`
...
Activate **mk:cook** skill: `/mk:cook <plan-path>` (interactive mode — review gates at each step)
