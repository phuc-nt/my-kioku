...
1. `mk:ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `mk:ui-ux-pro-max` - Design principles
3. `mk:frontend-design` - Implementation patterns
...
Analyze screenshots/videos with `mk:ai-multimodal` skill.
...
Screenshot + `mk:ai-multimodal` analysis.
...
Use `mk:agent-browser`, `mk:chrome-profile`, Chrome MCP / `chrome-devtools-mcp`, or project-native browser tests.
...
## Tips
- Use `mk:ai-multimodal` for generating visual assets
