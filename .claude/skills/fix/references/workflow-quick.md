...
Activate `mk:debug` skill. Activate `mk:sequential-thinking` for structured analysis.
...
|------|------------------|
| 1 | `mk:scout` (minimal) or direct file read |
| 2 | `mk:debug`, `mk:sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | `git-manager` subagent |

**Extra:** `mk:context-engineering` if dealing with AI/LLM code
