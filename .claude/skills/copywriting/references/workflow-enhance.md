...
1. **Screenshots provided** → Use `mk:ai-multimodal` skill to analyze and describe issues in detail
2. **Videos provided** → Use `mk:ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/mk:scout ext` (preferred) or `/mk:scout` to find files needing enhancement
