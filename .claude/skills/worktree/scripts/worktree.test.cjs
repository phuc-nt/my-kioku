...
const MONOREPO_DIR = '/home/kai/my-kit';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'my-agent-kit', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from my-agent-kit submodule
  const submodulePath = '/home/kai/my-kit/my-agent-kit';
...
test('create with partial project match in monorepo', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'my-kit-cli', 'Should find my-kit-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'my-kit' matches multiple projects
  const result = run('create my-kit test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
