...
- **Companion skill** — a `/mk:*` skill discoverable on the Claude Plugins Marketplace
...
Scope: converting existing code into CLI and/or MCP. Not for: building a server from scratch (use `/mk:mcp-builder`), raw npm scaffolding, or publishing without an agent-use story.

## Usage

```text
/mk:agentize [feature-or-module] [--both|--mcp|--cli] [--auto|--ask]
```
...
Invoke `/mk:project-management` **before** touching code. This skill owns plan/task lifecycle; `agentize` is a consumer.
...
Invoke `/mk:scout` to understand the target codebase. Without this, everything downstream is guessed.
...
1. **Tests** — invoke `/mk:test` to generate:
...
3. **Docs** — invoke `/mk:docs` to generate:
...
4. **Companion skill** — invoke `/mk:skill-creator` to generate:
...
Next: /mk:cook <plan-path> to execute any remaining implementation.
```
