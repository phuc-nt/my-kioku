...
const DEFAULT_USER_AGENT = 'my-agent-kit/1.0';
const DEFAULT_ELIGIBILITY_CACHE_TTL_MS = 60_000;

function getUsageCachePath() {
  return process.env.MK_USAGE_CACHE_PATH || path.join(os.tmpdir(), 'ck-usage-limits-cache.json');
}

function getQuotaEligibilityCachePath() {
  return process.env.MK_USAGE_ELIGIBILITY_CACHE_PATH || `${getUsageCachePath()}.eligibility`;
}
