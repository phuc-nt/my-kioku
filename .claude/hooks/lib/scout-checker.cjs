...
/**
 * Find an optional project-local .mkignore at the git root config directory.
...
function findProjectCkignore(startDir, configDirName) {
  ...
  const candidate = path.join(gitRoot, configDirName, '.mkignore');
 ...
 * @param {string} [params.options.mkignorePath] - Path to .mkignore file
 * @param {string} [params.options.projectCkignorePath] - Explicit project-local .mkignore path
 * @param {string} [params.options.claudeDir] - Path to .claude or .opencode directory
 * @param {string} [params.options.cwd] - Working directory used to discover a project .mkignore
...
function checkScoutBlock({ toolName, toolInput, options = {} }) {
  ...
  // Resolve .mkignore path
  const resolvedCkignorePath = ckignorePath || path.join(claudeDir, '.mkignore');
