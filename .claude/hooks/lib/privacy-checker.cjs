...
/**
 * Load .mk.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .mk.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.mk.json')
      : path.join(process.cwd(), '.claude', '.mk.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .mk.json config
