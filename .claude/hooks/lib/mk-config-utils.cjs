/**
 * Shared utilities for MyKit hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.mk.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.mk.json');
 ...
 * @param {Object} config - MyKit config
 ...
 *   2. Global config (~/.claude/.mk.json) - user preferences
 *   3. Local config (./.claude/.mk.json) - project-specific overrides
 *
