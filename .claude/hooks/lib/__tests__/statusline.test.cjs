...
test('getGitInfo timeout does not cache null result', () => {
  ...
  const originalTimeout = process.env.MK_GIT_TIMEOUT_MS;
  ...
  try {
    ...
    process.env.MK_GIT_TIMEOUT_MS = '50';
    ...
    if (originalTimeout == null) delete process.env.MK_GIT_TIMEOUT_MS;
    else process.env.MK_GIT_TIMEOUT_MS = originalTimeout;
