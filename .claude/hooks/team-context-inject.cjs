...
function buildCkContext() {
  ...
  if (env.MK_REPORTS_PATH) ctx.push(`Reports: ${env.MK_REPORTS_PATH}`);
  if (env.MK_PLANS_PATH) ctx.push(`Plans: ${env.MK_PLANS_PATH}`);
  if (env.MK_PROJECT_ROOT) ctx.push(`Project: ${env.MK_PROJECT_ROOT}`);
  if (env.MK_NAME_PATTERN) ctx.push(`Naming: ${env.MK_NAME_PATTERN}`);
  if (env.MK_GIT_BRANCH) ctx.push(`Branch: ${env.MK_GIT_BRANCH}`);
  if (env.MK_ACTIVE_PLAN) ctx.push(`Active plan: ${env.MK_ACTIVE_PLAN}`);
