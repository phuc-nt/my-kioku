 ...
 * Uses real .mkignore fixtures to validate end-to-end behavior.
 *
...
function createProjectRoot({ rootPatterns, nestedPatterns } = {}) {
  ...
  if (rootPatterns) {
    ...
    fs.writeFileSync(path.join(repoRoot, '.claude', '.mkignore'), `${rootPatterns.join('\n')}\n`);
  }
  if (nestedPatterns) {
    fs.mkdirSync(path.join(repoRoot, 'pkg', '.claude'), { recursive: true });
    fs.writeFileSync(path.join(repoRoot, 'pkg', '.claude', '.mkignore'), `${nestedPatterns.join('\n')}\n`);
  }
  return repoRoot;
}
...
describe('checkScoutBlock - fail-open behavior', () => {
  ...
  it('uses defaults when ckignorePath missing', () => {
    const r = checkScoutBlock({ toolName: 'Bash', toolInput: { command: 'ls node_modules' }, options: { ckignorePath: '/nonexistent/.mkignore' } });
    assert.ok(r.blocked, 'Should still block using DEFAULT_PATTERNS');
  });
});

describe('checkScoutBlock - custom .mkignore', () => {
  const customOpts = {
    ...DEFAULT_OPTS,
...
describe('checkScoutBlock - negation .mkignore', () => {
  const negOpts = {
    ...DEFAULT_OPTS,
...
describe('checkScoutBlock - project-local .claude/.mkignore discovery', () => {
  it('allows build paths via the git-root .claude/.mkignore override', () => {
  ...
  it('still blocks node_modules and points to the git-root .claude/.mkignore file', () => {
    const projectRoot = createProjectRoot({ rootPatterns: ['!build'] });
    const r = checkScoutBlock({
      toolName: 'Read',
    ...
    assert.ok(r.blocked);
    assert.strictEqual(r.configPath, path.join(projectRoot, '.claude', '.mkignore'));
  });
  ...
  it('ignores nested .claude/.mkignore files below the git root', () => {
