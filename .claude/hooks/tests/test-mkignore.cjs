...
/**
 * Test script for .mkignore functionality
 * Tests that scout-block.sh respects .mkignore patterns
 */
...
const ckignorePath = path.join(__dirname, '..', '..', '.mkignore');
const ckignoreBackupPath = ckignorePath + '.backup';

// Backup original .mkignore if exists
...
console.log('Testing .mkignore functionality...\n');

let passed = 0;
let failed = 0;

// Test 1: Default patterns work (with existing .mkignore)
console.log('--- Test 1: Default patterns from .mkignore ---');
let result = runTest(
...
console.log('\n--- Test 2: Custom .mkignore with only "vendor" ---');
...
result = runTest(
  'node_modules ALLOWED when not in .mkignore',
  { tool_name: 'Read', tool_input: { file_path: 'node_modules/pkg.json' } },
  'ALLOWED'
);
if (result.success) {
