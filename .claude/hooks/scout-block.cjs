 ...
 *
 * Blocks access to directories listed in the shipped .claude/.mkignore baseline
 * plus an optional project-local .claude/.mkignore override at the git root.
 ...
 *
 * Configuration:
 * - Edit .claude/.mkignore to customize shipped baseline patterns
 * - Add <project-root>/.claude/.mkignore to override locally without changing the baseline
  ...
  try {
    ...
    const result = checkScoutBlock({
      ...
      options: {
        ...
        ckignorePath: path.join(claudeDir, '.mkignore'),
