 ...
 *
 * Fires when Plan subagent completes. Reminds to invoke /mk:cook --auto before implementation.
...
try {
  ...
  try {
    ...
    const sessionId = process.env.MK_SESSION_ID;
    let planPath = null;
    ...
    console.log('MUST invoke /mk:cook --auto skill before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: Run /clear then /mk:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: Run /clear then /mk:cook {full-absolute-path-to-plan.md}');
    }
