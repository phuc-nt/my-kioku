...
function resolveArtifactDir({ cwd = process.cwd(), artifactDir = null, env = process.env } = {}) {
  ...
  const fromEnv = safeResolve(cwd, env.MK_WORKFLOW_ARTIFACT_DIR);
  if (fromEnv) return { artifactDir: fromEnv, source: 'env', reasons };
  if (env.MK_WORKFLOW_ARTIFACT_DIR) reasons.push('env artifact dir was invalid');
  ...
  if (fallback.length > 1) reasons.push('fallback is ambiguous; set MK_WORKFLOW_ARTIFACT_DIR');
