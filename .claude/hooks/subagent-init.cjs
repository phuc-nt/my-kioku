...
function buildPlanCliSection(agentType) {
  ...
  return [
    ...
    `\`mk plan check <id>\` = completed | \`mk plan check <id> --start\` = in-progress | \`mk plan uncheck <id>\` = revert`,
    `Fallback: if \`mk\` unavailable, edit plan.md Status column directly.`
  ];
}
...
async function main() {
  ...
  try {
    ...
    const sessionId = payload.session_id || process.env.MK_SESSION_ID || null;
