...
async function main() {
  ...
  try {
    ...
    if (envFile) {
      // Session & plan config
      writeEnv(envFile, 'MK_SESSION_ID', sessionId || '');
      writeEnv(envFile, 'MK_PLAN_NAMING_FORMAT', config.plan.namingFormat);
      writeEnv(envFile, 'MK_PLAN_DATE_FORMAT', config.plan.dateFormat);
      writeEnv(envFile, 'MK_PLAN_ISSUE_PREFIX', config.plan.issuePrefix || '');
      writeEnv(envFile, 'MK_PLAN_REPORTS_DIR', config.plan.reportsDir);

      // NEW: Resolved naming pattern for DRY file naming in agents
      // Example: "251212-1830-GH-88-{slug}" or "251212-1830-{slug}"
      // Agents use: `{agent-type}-$MK_NAME_PATTERN.md` and substitute {slug}
      writeEnv(envFile, 'MK_NAME_PATTERN', namePattern);

      // Plan resolution
      writeEnv(envFile, 'MK_ACTIVE_PLAN', resolved.resolvedBy === 'session' ? resolved.path : '');
      writeEnv(envFile, 'MK_SUGGESTED_PLAN', resolved.resolvedBy === 'branch' ? resolved.path : '');
      ...
      writeEnv(envFile, 'MK_GIT_ROOT', staticEnv.gitRoot || '');
      writeEnv(envFile, 'MK_REPORTS_PATH', path.join(baseDir, reportsPath));
      writeEnv(envFile, 'MK_DOCS_PATH', path.join(baseDir, config.paths.docs));
      writeEnv(envFile, 'MK_PLANS_PATH', path.join(baseDir, config.paths.plans));
      writeEnv(envFile, 'MK_PROJECT_ROOT', process.cwd());

      // Project detection
      writeEnv(envFile, 'MK_PROJECT_TYPE', detections.type || '');
      writeEnv(envFile, 'MK_PACKAGE_MANAGER', detections.pm || '');
      writeEnv(envFile, 'MK_FRAMEWORK', detections.framework || '');

      // NEW: Static environment info (so other hooks don't need to recompute)
      writeEnv(envFile, 'MK_NODE_VERSION', staticEnv.nodeVersion);
      writeEnv(envFile, 'MK_OS_PLATFORM', staticEnv.osPlatform);
      writeEnv(envFile, 'MK_GIT_BRANCH', staticEnv.gitBranch || '');
      writeEnv(envFile, 'MK_USER', staticEnv.user);
      writeEnv(envFile, 'MK_LOCALE', staticEnv.locale);
      writeEnv(envFile, 'MK_TIMEZONE', staticEnv.timezone);
      writeEnv(envFile, 'MK_CLAUDE_SETTINGS_DIR', staticEnv.claudeSettingsDir);

      // Locale config
      if (config.locale?.thinkingLanguage) {
        writeEnv(envFile, 'MK_THINKING_LANGUAGE', config.locale.thinkingLanguage);
      }
      if (config.locale?.responseLanguage) {
        writeEnv(envFile, 'MK_RESPONSE_LANGUAGE', config.locale.responseLanguage);
      }

      // Plan validation config (for /mk:plan validate, /mk:plan --hard, /mk:plan --parallel)
      const validation = config.plan?.validation || {};
      writeEnv(envFile, 'MK_VALIDATION_MODE', validation.mode || 'prompt');
      writeEnv(envFile, 'MK_VALIDATION_MIN_QUESTIONS', validation.minQuestions || 3);
      writeEnv(envFile, 'MK_VALIDATION_MAX_QUESTIONS', validation.maxQuestions || 8);
      writeEnv(envFile, 'MK_VALIDATION_FOCUS_AREAS', (validation.focusAreas || ['assumptions', 'risks', 'tradeoffs', 'architecture']).join(','));

      // Coding level config (for output style selection)
      const codingLevel = config.codingLevel ?? 5;
      writeEnv(envFile, 'MK_CODING_LEVEL', codingLevel);
      writeEnv(envFile, 'MK_CODING_LEVEL_STYLE', getCodingLevelStyleName(codingLevel));
    ...
    if (envFile && teamInfo) {
      writeEnv(envFile, 'MK_AGENT_TEAM', teamInfo.teamName);
      writeEnv(envFile, 'MK_AGENT_TEAM_MEMBERS', teamInfo.memberCount);
    }
