 ...
 *
 * Uses 'ignore' package for .mkignore parsing and path matching.
...
// Default patterns if .mkignore doesn't exist or is empty
...
function readPatternsFromFile(filePath) {
  ...
  try {
    ...
    console.error('WARN: Failed to read .mkignore:', error.message);
    return [];
  }
}

/**
 * Load patterns from the shipped .mkignore plus an optional project override.
 * Falls back to DEFAULT_PATTERNS if the shipped file doesn't exist or is empty.
 *
 * @param {string} ckignorePath - Path to shipped/global .mkignore file
 * @param {string} [projectCkignorePath] - Optional project-local .mkignore path
 ...
 *
 * @param {string[]} patterns - Array of patterns from .mkignore
 ...
 *
 * @param {string[]} originalPatterns - Original patterns from .mkignore
