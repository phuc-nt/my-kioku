 ...
 * Bypass via env MK_SIMPLIFY_DISABLED=1 or .mk.json hooks.simplify-gate=false.
 */
...
function loadConfig(cwd) {
  // Read .claude/.mk.json (canonical) or .mk.json (project root) scoped to cwd.
  let user = {};
  for (const rel of [path.join('.claude', '.mk.json'), '.mk.json']) {
    try {
...
function formatMessage(signals, breaches, severity) {
  ...
  return [
    ...
    `Bypass: set MK_SIMPLIFY_DISABLED=1 or reply 'force' to override.`
  ].join('\n');
}
...
function main() {
  if (process.env.MK_SIMPLIFY_DISABLED === '1') process.exit(0);
