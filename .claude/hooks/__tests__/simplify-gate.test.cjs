...
function makeRepo({ enableGate = true } = {}) {
  ...
  // so commit a .mk.json enabling it (committed so it doesn't pollute diff signals).
  // Tests that need default-off pass enableGate=false.
  if (enableGate) {
    fs.writeFileSync(path.join(dir, '.mk.json'), JSON.stringify({
...
test('MK_SIMPLIFY_DISABLED=1 short-circuits regardless of signals', () => {
  const dir = makeRepo();
  writeBigFile(dir, 'big.ts', 600);
  const r = runHook({ cwd: dir, prompt: 'ship it' }, { MK_SIMPLIFY_DISABLED: '1' });
  assert.strictEqual(r.status, 0);
  assert.strictEqual(r.stdout.trim(), '');
});

test('default config (no .mk.json) leaves gate OFF — exits silently even on big diff + ship verb', () => {
...
test('respects .mk.json simplify.gate.enabled=false', () => {
  const dir = makeRepo();
  writeBigFile(dir, 'big.ts', 600);
  fs.writeFileSync(path.join(dir, '.mk.json'), JSON.stringify({
...
test('honors custom thresholds from .mk.json', () => {
  const dir = makeRepo();
...
test('honors custom verbs from .mk.json', () => {
  const dir = makeRepo();
  writeBigFile(dir, 'big.ts', 600);
  fs.writeFileSync(path.join(dir, '.mk.json'), JSON.stringify({
