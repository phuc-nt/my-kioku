...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .mk.json', async () => {
      // Create .claude/.mk.json with hook disabled
      const claudeDir = path.join(tempDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
        path.join(claudeDir, '.mk.json'),
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'mk-config-utils.cjs'),
        path.join(libDir, 'mk-config-utils.cjs')
      );
