#!/usr/bin/env node
/**
 * Tests for session temp-state locking in mk-config-utils.cjs
...
const LIB_PATH = path.join(__dirname, '..', 'lib', 'mk-config-utils.cjs');
...
describe('mk-config-utils session state locking', () => {
