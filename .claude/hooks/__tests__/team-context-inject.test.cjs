...
describe('team-context-inject.cjs', () => {
    ...
    it('includes CK context when environment variables are set', async () => {
      ...
      try {
        ...
        const result = await runHook({
          ...
          env: {
            HOME: tmpDir,
            MK_REPORTS_PATH: '/project/plans/reports',
            MK_PLANS_PATH: '/project/plans',
            MK_PROJECT_ROOT: '/project',
            MK_GIT_BRANCH: 'main'
          }
        });
  ...
  describe('Error Handling & Robustness', () => {
    ...
    it('logs to stderr only when MK_DEBUG is enabled', async () => {
