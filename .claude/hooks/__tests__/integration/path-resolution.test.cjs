...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('scout-block uses payload.cwd to discover a git-root .claude/.mkignore', async () => {
      ...
      fs.writeFileSync(
        path.join(tempHome, '.claude', '.mk.json'),
      ...
      fs.writeFileSync(path.join(projectRoot, '.claude', '.mkignore'), '!build\n');
      ...
      try {
        ...
        assert.ok(
          blockResult.stderr.includes(path.join(projectRoot, '.claude', '.mkignore')),
